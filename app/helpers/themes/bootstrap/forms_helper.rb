...
module Themes::Bootstrap::FormsHelper
  def form_header(options = {})
    ...
    header.html_safe #content_tag (options[:style] || :h1), header.html_safe
  end
  ...
  def form_for(record, options = {}, &block) 
    ...
    if options[:wrapper]
      content_tag :div, class: 'ui grid' do
        content_tag(:div, raw(hdr + super), class: 'ten wide column') + content_tag(:div, '', class: 'six wide column')
      end
    else
      (hdr + super).html_safe
    end
