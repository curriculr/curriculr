...
module Themes::Bootstrap::UiComponentsHelper
  ...
  # def ui_floated_side_by_side(side_a, side_b, config = 'twelve by four')
  #   sizes = config.split('by').map{|s| s.strip}
  #   content_tag :div, class: "ui two column grid" do
  #     html = ''
  #     html << content_tag(:div, side_a, class: "left floated #{sizes[0]} wide column") unless sizes[0] == 'zero'
  #     html << content_tag(:div, side_b, class: "right floated #{sizes[1]} wide column") unless sizes[1] == 'zero'
  #
  #     html.html_safe
  #   end
  # end
