class Klass < ActiveRecord::Base
  ...
  scope :being_worked_on, -> { where("klasses.active = TRUE and klasses.ready_to_approve = FALSE and klasses.approved = FALSE") }
  scope :ready_to_approve, -> { where("klasses.active = TRUE and klasses.ready_to_approve = TRUE and klasses.approved = FALSE") }
  ...
  scope :for_instructor, ->(user) {
    joins(:course).
    joins('left outer join instructors ON instructors.course_id = courses.id').
    where('courses.originator_id = :user_id or instructors.user_id = :user_id', :user_id => user.id)
  }
