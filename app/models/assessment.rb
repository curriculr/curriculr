class Assessment < ActiveRecord::Base
  ...
  def proper_attempts
    if allowed_attempts.present? and droppable_attempts >= allowed_attempts
      errors.add :droppable_attempts, :less_than_or_equal_to, :count => allowed_attempts
    end
  end
  
  def proper_from_and_to_datetimes
    if from_datetime.present? and from_datetime.to_date < based_on
      errors.add :from_datetime, :must_be_after_date, :date => based_on
    end 
    
    if from_datetime.present? and to_datetime.present? and from_datetime >= to_datetime
      errors.add :to_datetime, :must_be_after_start_date
    end 
  end  
  
  def proper_invideo
    if invideo_id.present? and invideo_at.blank? 
      errors.add :invideo_at, :blank
    end 
    
    if invideo_at.present? and invideo_id.blank? 
  ...
  def show_answer?(klass)
    if %w(during_attempt after_attempt always).include?(show_answer)
      return true
    elsif show_answer == 'after_deadline' and to_datetime.present?
