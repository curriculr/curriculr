...
Mailer.klass_invitation(account.slug, from, to, 1, Faker::Lorem.words(2).join(' '), 'http://localhost/sign_in').deliver_now

Mailer.klass_enrollment(account.slug, from, to, [1,2,3], 'http://localhost/sign_in').deliver_now
...
Mailer.klass_update(account.slug, from, to, Faker::Lorem.words(4).join(' '), body, 2).deliver_now
